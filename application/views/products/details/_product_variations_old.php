                ...
                foreach ($variation_options as $option):
                   ...
                   // $option->option_names = get_variation_option_name($option->option_names, $this->selected_lang->id); ?>
                    <div class="custom-control custom-control-variation custom-control-validate-input">
                        <input type="radio" name="variation<?php echo $variation->id; ?>" data-name="variation<?php echo $variation->id; ?>" value="<?php echo $option->id; ?>" id="radio<?php echo $option->id; ?>" class="custom-control-input form-check-input" <?php echo ($option->is_default == 1) ? 'checked' : ''; ?> onchange="select_product_variation_option('<?php echo $variation->id; ?>', 'radio_button', $(this).val());" required>
                        ...
                        <?php else: ?>
                            <label for="radio<?php echo $option->id; ?>" data-input-name="variation<?php echo $variation->id; ?>" class="custom-control-label label-variation-size label-variation-color label-variation<?php echo $variation->id; ?> <?php echo ($option_stock < 1) ? 'option-out-of-stock' : ''; ?>">
    ...
    <?php elseif ($variation->variation_type == 'dropdown'): ?>
        <div class="col-12 col-lg-6 col-product-variation item-variation">
            <div class="form-group">
                <label class="control-label"><?php echo $variation->label_names; ?></label>
                <div class="selectdiv">
                          ...
                          //  $variation_options = get_product_variation_options($variation->id);
                            if (!empty($variation_options)):
                                foreach ($variation_options as $option):
                                    ...
                                    $option->option_names = get_variation_option_name($option->option_names, $this->selected_lang->id); ?>
                                    <option value="<?php echo $option->id; ?>" <?php echo ($option_stock < 1) ? 'disabled' : ''; ?> <?php echo ($option->is_default == 1) ? 'selected' : ''; ?>><?php echo html_escape($option->option_names); ?></option>
                                <?php endforeach;
                            endif;
                        else: ?>
                            ...
                            if (!empty($default_option)):
                                ...
                                if (!empty($sub_options)):
                                    foreach ($sub_options as $sub_option):
                                        $option->option_names = get_variation_option_name($sub_option->option_names, $this->selected_lang->id); ?>
                                        <option value="<?php echo $sub_option->id; ?>"><?php echo html_escape($option->option_names); ?></option>
                                    <?php endforeach;
                                endif;
                            endif;
                        endif; ?>
        ...
        <div class="col-12 col-product-variation product-variation-checkbox">
            ...
            if (!empty($variation_options)):
                foreach ($variation_options as $option):
                    ...
                    $option->option_names = get_variation_option_name($option->option_names, $this->selected_lang->id); ?>
                    <div class="custom-control custom-control-variation custom-control-validate-input">
                        <input type="checkbox" name="variation<?php echo $variation->id; ?>[]" value="<?php echo $option->id; ?>" id="checkbox<?php echo $option->id; ?>" class="custom-control-input form-check-input" required>
                        <?php if ($variation->option_display_type == 'image'):
                            $option_image = get_variation_main_option_image_url($option, $product_images); ?>
                        ...
                        <?php elseif ($variation->option_display_type == 'color'): ?>
                            <label for="checkbox<?php echo $option->id; ?>" data-input-name="variation<?php echo $variation->id; ?>" class="custom-control-label label-variation-color label-variation<?php echo $variation->id; ?> <?php echo ($option_stock < 1) ? 'option-out-of-stock' : ''; ?>" data-toggle="tooltip" data-placement="top" title="<?php echo html_escape($option->option_names); ?>">
                                <span class="variation-color-box" style="background-color: <?php echo $option->color; ?>"></span>
                            </label>
                        <?php else: ?>
