...
<?php if ($cart_payment_method->payment_option == 'razorpay'):
	...
	if (!empty($razorpay_order_id)): ?>
		...
		<div id="payment-button-container" class=paypal-button-cnt">
			<p class="p-complete-payment payment-processing">Please donot referesh the page Your Payment is Processing...</p>
			<button type="button" id="rzp-button1" class="btn btn-lg custom-stripe-button" style="display:none;">Pay Now</button>
		</div>
		...
		<script>
            var options = {
                ...
                "description": "Pay",
                "image": "<?= get_image($this->general_settings->logo);?>",
                "order_id": "<?php echo $razorpay_order_id; ?>",
                "prefill":{
                    "name":"<?= $this->auth_user->full_name;?>",
                    "email":"<?= $this->auth_user->email;?>",
                    "contact":"<?= $this->auth_user->phone_number;?>"
                },
