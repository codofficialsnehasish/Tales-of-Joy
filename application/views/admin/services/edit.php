<div class="page-content">
   <div class="container-fluid">
      ...
      <!-- end page title -->
      <?= form_open_multipart('admin/services/update-process', 'class="custom-validation"');?>
               ...
               <div class="card">
                     ...
                     <label class="form-label">Parent Category</label>
                        <div>
                           <select class="form-select" name="cat_id" id="cat_id" aria-label="Default select example" required>
                                 ...
                                 <?php foreach($service_parentcategory as $category):?>
                              <option value="<?= $category->cat_id?>" <?= $item->cat_id==$category->cat_id?'selected':'';?>><?= $category->cat_name;?></option>
                              <?php endforeach;?>
                           </select>
                        </div>
                  </div>
                        ...
                        <div>
                           <select class="form-select" name="subcat_id" id="subcat_id" aria-label="Default select example">
                                 ...
                                 <?php foreach($subcategories as $subcat):?>
                                    <option value="<?= $subcat->cat_id;?>" <?= $item->subcat_id==$subcat->cat_id?'selected':'';?>><?= $subcat->cat_name;?></option>
                                 <?php endforeach;?>
                                 </select>
                        </div>
                  </div>
                  <div class="mb-3">
                        ...
                        <input data-parsley-type="text" type="text"
                              ...
                              placeholder="Enter Title" name="title" value="<?= $item->title;?>">
                        </div>
                     </div>      
                     <div class="mb-3">
                        <label class="form-label">Description</label>
                        <div>
                           <textarea name="service_desc"  class="form-control editor" rows="5"><?= $item->service_desc;?></textarea>
                        </div>
                     </div>                    
