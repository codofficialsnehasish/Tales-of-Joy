...
{    
	...
	{  
		if ($this->permission) { 
			...
			$this->ci->session->set_flashdata('error', "You do not have permission to access. Please contact with administrator.");
	...
	{ 
		$permission = $this->ci->session->userdata('modesy_sess_permission');
		$isAdmin    = $this->ci->session->userdata('isAdmin');
		$isLogIn    = $this->ci->session->userdata('modesy_sess_logged_in');
