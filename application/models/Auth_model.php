...
class Auth_model extends CI_Model
{
    ...
    {
        if($this->input->post('reg_opt',true) == 'dristributor'){
            $data=array(
                // 'login_type'=>$this->input->post('login_type',true),
                'first_name'=>$this->input->post('first_name',true),
                'last_name'=>$this->input->post('last_name',true),
                'role'=> $this->input->post('reg_opt',true),
                'address'=>$this->input->post('address',true),
                'zip_code'=>$this->input->post('pin_number',true),
        ...
        }else if($this->input->post('reg_opt',true) == 'teamlead'){
            $data=array(
                // 'login_type'=>$this->input->post('login_type',true),
                'first_name'=>$this->input->post('first_name',true),
                'last_name'=>$this->input->post('last_name',true),
                ...
                'address'=>$this->input->post('address',true),
                'zip_code'=>$this->input->post('pin_number',true),
                'prefarable_zip_code'=>$this->input->post('pref_pin_number',true),
                'pan_no'=>$this->input->post('pan_no',true),
                'aadhar_no'=>$this->input->post('aadhar_no',true),
            //    'country_id'=>$this->input->post('country_id',true),
            //    'state_id'=>$this->input->post('state_id',true),
        ...
        }else if($this->input->post('reg_opt',true) == 'dristributorbytl'){
            $data=array(
                // 'login_type'=>$this->input->post('login_type',true),
    ...
    public function login()
    {
        ...
        if (!empty($user)) {
            ...
            if(($user->role == 'teamlead' || $user->role == 'dristributor') && $user->is_approved == 1){
                ...
                $this->session->set_flashdata('error', 'You are not approved for this role, Wait a while !');
                return false;
            }
        } else {
